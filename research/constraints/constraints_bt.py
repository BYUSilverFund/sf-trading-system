...
if __name__ == "__main__":
    ...
    constraints_set_configs = [
        {"name": "full_investment", "constraints": [full_investment]},
        {
            "name": "leverage_constrained",
            "constraints": [full_investment, no_buying_on_margin, long_only],
        },
        {
            "name": "unit_beta",
            "constraints": [
    ...
    for config in constraints_set_configs:
        # Define strategy
        strategy = Strategy(
        ...
        # Save results
        folder = Path("research/constraints/results")
        os.makedirs(folder, exist_ok=True)
        asset_returns.write_parquet(folder / f"{config.get('name')}_bt.parquet")
