"""
REMOVE ONCE MOVE TO MAIN CONFIRMED
...
def rebalance_port(optimal_wts_df):
    \"""
    ...
    optimal_wts_df['ticker'] = [' '.join(ticker.split('.')) for ticker in optimal_wts_df['ticker'].values]
    ...
    optimal_wts_df = pd.merge(optimal_wts_df, holdings_df[['marketPrice']], how='outer',left_index=True,right_index=True)
    ...
    order_df = pd.merge(order_df, holdings_df[['position']], how='outer', left_index=True, right_index=True).fillna(0)
    ...
    order_df = pd.merge(order_df, get_security_info(order_df.index), how='inner', left_index=True, right_index=True)
