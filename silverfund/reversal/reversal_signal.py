...
def classic_reversal(data: pl.DataFrame) -> Signal:
    signals = (
        data.with_columns((pl.col("ret") * (-1)).alias("rev"))
        .select(["date", "barrid", "rev"])
        .sort(["barrid", "date"])
    )
    return Signal(signals, "rev")


def daily_reversal(data: pl.DataFrame) -> Signal:
    signals = (
        data.with_columns((pl.col("ret") * (-1)).rolling_sum(22).over("barrid").alias("rev"))
        .select(["date", "barrid", "rev"])
        .sort(["barrid", "date"])
    )
    return Signal(signals, "rev")
...
def barra_spec_reversal(data: pl.DataFrame) -> Signal:
    signals = (
        data.with_columns((pl.col("spec_ret") * (-1)).alias("rev"))
        .select(["date", "barrid", "rev"])
        .sort(["barrid", "date"])
    )
    return Signal(signals, "rev")
...
def daily_barra_spec_reversal(data: pl.DataFrame) -> Signal:
    signals = (
        data.with_columns((pl.col("spec_ret") * (-1)).rolling_sum(22).over("barrid").alias("rev"))
        .select(["date", "barrid", "rev"])
        .sort(["barrid", "date"])
    )
    return Signal(signals, "rev")
